...
public class AddTextHandler implements RequestStreamHandler {
	...
	boolean addText(String eventtype, String recipient, String orientation, int left, int top, int width, int height, String text, int page, String font) throws Exception {
		if (logger != null) { logger.log("in addText"); }
		AddTextDB atd = new AddTextDB();
		
		// check if present
		if (atd.addText(eventtype, recipient, orientation, left, top, width, height, text, page, font)){
			return true;
		}
		else {
			return false;
	...
	@Override
	  public void handleRequest(InputStream input, OutputStream output, Context context) throws IOException {
		...
		if (error) {
			...
			try {
				if (addText(eventtype, recipient, orientation, left, top, width, height, text, page, font)) {
					statusCode = 200;
				}
				else {
					statusCode = 409;
